...
실제로 구현. 바로 위 항목(캐릭터 닉네임)에서 닉네임 필드를 미리 만들어둔 덕에 바로 진행 가능했다.

**정렬 기준 결정**: 골드는 강화/소환에 써서 줄어들 수 있어 랭킹 기준으로 부적합(순위가 오르내리며
요동침) — 대신 한 번 오르면 절대 안 내려가는 `maxClearedStage`(최고 클리어 사냥터)를 1차 기준,
`level`을 2차 동점 기준, `updatedAt` 오름차순(먼저 달성한 사람이 위)을 3차 동점 기준으로 삼았다.
"내 순위 강조" 같은 부가 기능은 요청 범위 밖이라 넣지 않음(과설계 방지).

**백엔드**: `server/src/ranking/` 신설(모듈/컨트롤러/서비스). `Save.data`가 JSONB라 Prisma의
`orderBy`로 JSON 경로 정렬이 안 되어 `$queryRaw` 사용. 인증 가드 없음(게스트도 랭킹 열람 가능,
로그인 안 하면 자기 기록만 안 나옴). 닉네임 필드 도입 전(v4 이하) 저장된 옛 세이브를 위해
`COALESCE`로 "익명의 모험가"/레벨1/스테이지1 기본값 처리 — 실제로 로컬 DB에 있던 예전 테스트
계정(pam9411 등)으로 이 폴백이 정확히 동작하는 걸 확인함.

**프론트**: `HuntNavBar`에 7번째 탭 "랭킹" 추가. 7개 탭이 좁은 화면에서 넘치거나 찌그러지지
않을지 걱정했는데 Playwright 모바일 뷰포트(Pixel 7) 스크린샷으로 직접 확인해보니 문제없었다
(`justify-content: space-around`가 gap을 줄여 흡수). `RankingPanel`은 `GameSheet`의 `v-else-if`
분기 안에 있어서 시트를 닫을 때마다 언마운트되고 다시 열 때 `onMounted`에서 새로 fetch —
"열 때마다 새로 조회" 요구사항을 자연스럽게 만족(별도 새로고침 버튼 불필요).

**테스트 전략**: 다른 백엔드 연동 기능과 동일하게, 커밋되는 프론트 e2e 스펙(`ranking.spec.ts`)은
백엔드 없이도 통과하게(탭 진입 + 에러 상태만 검증) 만들고, 실제 정렬 로직 검증은
`server/test/ranking.e2e-spec.ts`(백엔드 Jest e2e, 실제 Postgres 필요)에 맡겼다. 로컬에서 실제
백엔드를 띄워 스크린샷까지 찍어 골드/실버/브론즈 랭크 색상, 닉네임/스테이지/레벨 표시가 실제
데이터로 정확히 렌더링되는 걸 확인.

**배포**: 백엔드는 Render에 push하면 자동 배포(`render.yaml`의 buildCommand가 `prisma migrate
deploy`도 포함하지만, 이번 변경은 스키마 변경이 없어 마이그레이션은 없음 — `Save.data` JSONB
안의 필드라 스키마 자체는 그대로). 프론트는 Vercel 자동 배포.
